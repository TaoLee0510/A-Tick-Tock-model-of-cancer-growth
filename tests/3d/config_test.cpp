...
int main() {
    ...
    assert(smoke.profile == "smoke_test_v2");
    ...
    assert(smoke.initial_migration_rate_model == "fixed");
    assert(smoke.initial_r_migration_rate == 0.25 && smoke.initial_K_migration_rate == 0.25);
    ...
    assert(production.profile == "legacy_2d_mapped_v2");
    ...
    assert(production.initial_migration_rate_model == "legacy_beta_v1");
    assert(production.initial_r_migration_beta.alpha == 0.01);
    assert(production.initial_r_migration_beta.beta == 0.0566666667);
    assert(production.initial_r_migration_beta.scale == 200.0);
    assert(production.initial_r_migration_beta.lower_clamp_enabled);
    assert(production.initial_r_migration_beta.lower_clamp_threshold == 0.5);
    assert(production.initial_r_migration_beta.lower_clamp_value == 0.25);
    ...
    assert(production.to_json().find(
        ...
        std::string::npos);
    assert(production.to_json().find(
        "\"initial_migration_rate_model\":\"legacy_beta_v1\"") !=
        std::string::npos);
    Model3DConfig changed_rate = production;
    changed_rate.initial_r_migration_beta.alpha += 0.001;
    ...
    expect_rejected("checkpoint_v1",
                    replace_once(smoke_yaml, "  checkpoint_format: hdf5_v2\n",
