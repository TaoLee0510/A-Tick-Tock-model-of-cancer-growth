...
int main(int argc, char** argv) {
    try {
        ...
        int threads = 1;
        std::string scheduler = "event_queue_v1";
        double priority_bucket_hours = 0.01;
        std::uint64_t min_events_per_thread = 1024;
        std::uint64_t min_refresh_items_per_thread = 16;
        bool activated_density = false;
        for (int index = 1; index < argc; ++index) {
            ...
            else if (argument == "--scheduler" && index + 1 < argc) scheduler = argv[++index];
            else if (argument == "--priority-bucket-hours" &&
                     index + 1 < argc) {
                priority_bucket_hours = std::stod(argv[++index]);
            } else if (argument == "--min-events-per-thread" &&
                       index + 1 < argc) {
                min_events_per_thread = std::stoull(argv[++index]);
            } else if (argument == "--min-refresh-items-per-thread" &&
                       index + 1 < argc) {
                min_refresh_items_per_thread =
                    std::stoull(argv[++index]);
            } else if (argument == "--activated-density") {
                activated_density = true;
            }
            else throw std::invalid_argument("unknown or incomplete argument: " + argument);
        }
        if (scheduler == "exact") scheduler = "event_queue_v1";
        if (scheduler == "windowed") {
            scheduler = "deterministic_exact_window_v3";
        }
        if (cells < 2 || max_events == 0 || threads <= 0 ||
            !(priority_bucket_hours > 0.0) ||
            !std::isfinite(priority_bucket_hours) ||
            min_events_per_thread == 0 ||
            min_refresh_items_per_thread == 0 ||
            (scheduler != "event_queue_v1" &&
             scheduler != "deterministic_exact_window_v3")) {
        ...
        config.migration_activation_enabled = activated_density;
        config.angiogenesis.enabled = false;
        config.end_time_hours = 1000000.0;
        config.max_events = max_events;
        config.threads = threads;
        config.scheduler_backend = scheduler;
        config.conflict_bucket_hours = priority_bucket_hours;
        config.parallel_min_events_per_thread =
            static_cast<std::size_t>(min_events_per_thread);
        ...
        for (std::uint64_t index = 0; index < cells; ++index) {
            ...
            cell.anchor = {
                ...
                static_cast<std::int32_t>(3ULL * (index / (side * side)))};
            cell.flags = static_cast<std::uint8_t>(
                atcg3d::kDirtyDensity |
                (activated_density ? atcg3d::kMigrationActive : 0));
            ...
            cell.migration_activation_end_time =
                activated_density ? 1000000.0 : 0.0;
            cell.division_work_remaining = 1000000.0F;
            const double unquantized_time =
                0.01 + 0.99 * atcg3d::rng_unit(
                config.seed, cell.uid, 0x45564e5442454e43ULL, 0, 0);
            cell.next_migration_time = unquantized_time;
        ...
        std::cout << std::setprecision(17)
                  ...
                  << "  \"conflict_priority_bucket_hours\": "
                  << priority_bucket_hours << ",\n"
                  << "  \"minimum_events_per_thread\": "
                  << min_events_per_thread << ",\n"
                  << "  \"minimum_refresh_items_per_thread\": "
                  << min_refresh_items_per_thread << ",\n"
                  << "  \"activated_density\": "
                  << (activated_density ? "true" : "false") << ",\n"
                  << "  \"maximum_workers\": "
                  << simulation.proposal_window_diagnostics().maximum_workers
                  << ",\n"
                  << "  \"proposal_windows\": "
                  << simulation.proposal_window_diagnostics().windows << ",\n"
                  << "  \"prefetched_migration_proposals\": "
                  << simulation.proposal_window_diagnostics().migration_proposals
                  << ",\n"
                  << "  \"proposal_cache_hits\": "
                  << simulation.proposal_window_diagnostics().migration_cache_hits
                  << ",\n"
                  << "  \"proposal_cache_invalidations\": "
                  << simulation.proposal_window_diagnostics()
                         .migration_cache_invalidations
                  << ",\n"
                  << "  \"proposal_cache_misses\": "
                  << simulation.proposal_window_diagnostics()
                         .migration_cache_misses
                  << ",\n"
