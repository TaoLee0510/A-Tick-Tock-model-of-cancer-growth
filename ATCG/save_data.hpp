...
inline void save_data(int Visual_range_x, int Visual_range_y, int N0, int N00, int N01, int MMR, int H, int &T, double alpha, double beta, const CellArray &cell_array, int migration_judgement,double deltah, const Array<double,2> &colorspace, int DDM, int allpng)
{
    if (H%MMR==0)
    {
        if (DDM==1 && T>=1  && migration_judgement==0)
        {
