...
inline std::uint64_t stateless_splitmix64(std::uint64_t x)
{
...
inline unsigned long stateless_seed(long cell_id, long time_step, long event_id)
{
...
inline double stateless_uniform(long cell_id, long time_step, long event_id)
{
    std::mt19937_64 engine(stateless_seed(cell_id, time_step, event_id));
    return std::generate_canonical<double, 53>(engine);
}

inline double stateless_beta(long cell_id, long time_step, long event_id, double alpha, double beta)
{
...
inline unsigned int stateless_geometric(long cell_id, long time_step, long event_id, double p)
{
    if (p >= 1.0)
    {
        return 1;
...
inline void stateless_shuffle(RandomIt first, RandomIt last, long cell_id, long time_step, long event_id)
{
    std::mt19937_64 engine(stateless_seed(cell_id, time_step, event_id));
    std::shuffle(first, last, engine);
}

#endif /* stateless_rng_hpp */
