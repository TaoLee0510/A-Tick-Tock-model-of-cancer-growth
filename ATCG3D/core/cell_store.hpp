...
public:
    ...
    double next_migration_time(Slot slot) const { return next_migration_time_.at(slot); }
    double next_division_time(Slot slot) const { return next_division_time_.at(slot); }
    double death_deadline(Slot slot) const { return static_cast<double>(death_deadline_.at(slot)); }
    double last_update_time(Slot slot) const { return last_update_time_.at(slot); }
    std::uint64_t event_sequence(Slot slot) const { return event_sequence_.at(slot); }
    std::uint32_t schedule_generation(Slot slot) const { return schedule_generation_.at(slot); }
    ...
    void set_stage(Slot slot, CellStage value) { stage_.at(slot) = static_cast<std::uint8_t>(value); }
    ...
    void set_next_migration_time(Slot slot, double value) { next_migration_time_.at(slot) = value; }
    void set_next_division_time(Slot slot, double value) { next_division_time_.at(slot) = value; }
    void set_death_deadline(Slot slot, double value) { death_deadline_.at(slot) = static_cast<float>(value); }
    void set_last_update_time(Slot slot, double value) { last_update_time_.at(slot) = value; }
    ...
    void set_schedule_generation(Slot slot, std::uint32_t value) { schedule_generation_.at(slot) = value; }
    std::uint32_t bump_schedule_generation(Slot slot) { return ++schedule_generation_.at(slot); }

    std::size_t allocated_bytes() const noexcept;
    static constexpr std::size_t logical_bytes_per_slot() noexcept {
        return sizeof(std::int32_t) * 3 + sizeof(CellUid) * 2 + sizeof(std::uint32_t) * 2 +
               sizeof(std::uint8_t) * 6 + sizeof(float) * 4 + sizeof(double) * 3 +
               sizeof(std::uint64_t);
    }
...
private:
    ...
    std::vector<double> next_migration_time_;
    std::vector<double> next_division_time_;
